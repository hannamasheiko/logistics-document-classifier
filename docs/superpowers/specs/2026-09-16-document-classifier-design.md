...
Документ узагальнює погоджений дизайн із розмови. Він не є implementation plan. `AI_WORKFLOW.md` / `AI_WORKFLOW_UA.md` залишаються historical snapshots Brainstorming, Planning і Task 1 та більше не підтримуються. Кандидати та відкладені рішення позначені явно; їх не слід вважати обраною реалізацією.
